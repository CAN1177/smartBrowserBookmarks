...
const MESSAGES = {
  zh_CN: {
    ...
    defaultBookmarksCollapsedTooltip: '影响主界面所有文件夹的\'书签\'区块初始展开/折叠状态',
    ...
    difyBaseUrlTooltip: '通常为 https://api.dify.ai；如使用自托管或私有部署，请填写对应地址',
    ...
    backToTop: '回到顶部',
    tips: '小贴士',
    createFolderGuide: '在浏览器中创建书签文件夹，然后刷新此页面即可开始管理您的智能书签',
    directory: '目录',
    expandMore: '展开更多',
  ...
  en: {
    ...
    defaultBookmarksCollapsedTooltip: 'Affects the initial expand/collapse state of \'Bookmarks\' sections in all folders in the main interface',
    useDifyKeyword: 'Use Dify to Generate Keywords',
    difyApiKey: 'Dify API Key',
    difyApiKeyTooltip: 'API Key obtained from Dify console, used to call workflow interface',
    difyBaseUrl: 'Dify API Base URL',
    difyBaseUrlTooltip: 'Usually https://api.dify.ai; if using self-hosted or private deployment, please fill in the corresponding address',
    difyUserId: 'Dify User ID',
    ...
    previewEnvironment: 'Current preview environment, settings not written to browser',
    ...
    folderEmptyHint: 'This folder has no content yet. Add bookmarks from your browser.',
    save: 'Save',
    ...
    tips: 'Tips',
    createFolderGuide: 'Create bookmark folders in your browser, then refresh this page to start managing your smart bookmarks.',
    directory: 'Directory',
...
export async function getCurrentLanguage(): Promise<SupportedLanguage> {
  ...
  try {
    if (typeof window !== 'undefined' && window.localStorage) {
      const localLang = window.localStorage.getItem(LANGUAGE_STORAGE_KEY) as SupportedLanguage | null;
      if (localLang) {
  ...
  if (typeof chrome !== 'undefined' && chrome.storage) {
    try {
      ...
      const savedLanguage = (result[LANGUAGE_STORAGE_KEY] as SupportedLanguage) || DEFAULT_LANGUAGE;
